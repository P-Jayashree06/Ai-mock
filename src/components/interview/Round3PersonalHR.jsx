...
import { saveQuestionResult, saveFinalResult } from '../../lib/instantdb';
import { evaluateHRAnswer, generatePersonalHRQuestions } from '../../lib/kimiAPI';
import { useCamera } from '../../hooks/useCamera';
import { useTimer } from '../../hooks/useTimer';
import Loader from '../shared/Loader';
import { getMaleVoice } from '../../lib/voiceUtils';

export default function Round3PersonalHR() {
  const navigate = useNavigate();
  const { 
    resumeAnalysis, currentInterviewId, 
    personalHRQuestions, setPersonalHRQuestions, 
    currentQuestionIndex, submitPersonalHRAnswer, 
    nextQuestion, completeRound3, round1Score, round2Score 
  } = useInterviewStore();
  ...
  useEffect(() => {
    ...
    if (personalHRQuestions.length === 0 && !isGenerating) {
      const generate = async () => {
        setIsGenerating(true);
        try {
          const qs = await generatePersonalHRQuestions(resumeAnalysis);
          setPersonalHRQuestions(qs);
        } catch (err) {
  ...
  }, [resumeAnalysis, currentInterviewId, personalHRQuestions, startCamera, setPersonalHRQuestions, navigate]);

  // TTS Voice Output & Timer Init
  useEffect(() => {
    if (personalHRQuestions.length > 0 && !evalResult) {
      const q = personalHRQuestions[currentQuestionIndex];
  ...
  }, [personalHRQuestions, currentQuestionIndex, evalResult, startTimer]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      submitPersonalHRAnswer(q.id, answerText, result);
  ...
  const handleNext = async () => {
    ...
    if (currentQuestionIndex < personalHRQuestions.length - 1) {
      nextQuestion();
    } else {
      const subs = useInterviewStore.getState().personalHRAnswers;
      const avg = Math.round(subs.reduce((acc, sub) => acc + sub.evaluation.score, 0) / subs.length);
      completeRound3(avg);
      stopCamera();
      
      const state = useInterviewStore.getState();
      const finalOverallScore = Math.round((state.round1Score + state.round2Score + avg) / 3);
      
      await saveFinalResult(currentInterviewId, {
  ...
  if (isGenerating || personalHRQuestions.length === 0) {
    return <div className="min-h-screen bg-bg-primary flex"><Loader variant="Generating" /></div>;
  }

  const question = personalHRQuestions[currentQuestionIndex];
    ...
    <div className="min-h-screen bg-bg-primary py-8 px-4 sm:px-6 lg:px-8 flex flex-col max-w-5xl mx-auto space-y-6">
      <div className="flex justify-between items-center bg-bg-card p-4 rounded-xl border border-subtle">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-display font-bold">Round 3: Personal HR</h2>
            <p className="text-text-secondary text-sm">Question {currentQuestionIndex + 1} of {personalHRQuestions.length}</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
        ...
        <div className="w-full lg:w-1/2 flex flex-col relative glass-card overflow-hidden">
          ...
          <div className="p-4 border-t border-subtle bg-bg-secondary mt-auto">
            {evalResult ? (
              ...
              >
                {currentQuestionIndex < personalHRQuestions.length - 1 ? 'Next Question' : 'Complete Interview'} <ChevronRight className="w-5 h-5 ml-2" />
              </button>
            ) : (
              <button
